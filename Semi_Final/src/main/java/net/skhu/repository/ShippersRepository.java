...
import net.skhu.domain.Shippers;
...
public interface ShippersRepository extends JpaRepository<Shippers,Integer> {
	...
	public default List<Shippers> findAll(Pagination pagination){
	...
	@Override
	public Page<Shippers> findAll(Pageable pageable);

	@Query("select s from Shippers s where s.lastName like CONCAT('%',:name,'%') or s.firstName like CONCAT('%',:name,'%')")
	public Page<Shippers> findByName(@Param("name") String name,Pageable pageable);

	public Page<Shippers> findByJobTitleContains(String jobTitle, Pageable pageable);
	public Page<Shippers> findByCityStartsWith(String city, Pageable pageable);
}
